...
// === DATA SAVER / REDUCED DATA PREFERENCE ===
(function applyReducedDataPreference(){
    try {
        const root = document.documentElement;
        const conn = navigator.connection || navigator.mozConnection || navigator.webkitConnection;
        const prefersReduced = !!(conn && (conn.saveData || (conn.effectiveType && /^(slow-2g|2g)$/.test(conn.effectiveType))));
        if (prefersReduced) {
            root.classList.add('reduced-data');
        } else {
            root.classList.remove('reduced-data');
        }
        if (conn && typeof conn.addEventListener === 'function') {
            conn.addEventListener('change', () => {
                const isReduced = !!(conn.saveData || (conn.effectiveType && /^(slow-2g|2g)$/.test(conn.effectiveType)));
                root.classList.toggle('reduced-data', isReduced);
            });
        }
    } catch (e) {
        // Silently ignore if the Network Information API is not available
    }
})();
...
// WINE FILTER FUNCTIONALITY - FINAL FIXED VERSION
...
if (filterBtns.length > 0 && (wineCardsFilter.length > 0 || wineSections.length > 0)) {
    filterBtns.forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            // FIRST: Hide/show sections immediately
            if (filterValue === 'all') {
            ...
            // THEN: Wait for layout to update, THEN scroll
            requestAnimationFrame(() => {
                requestAnimationFrame(() => {
                    const firstVisibleSection = filterValue === 'all'
                        ? document.querySelector('.wines-catalogue-section')
                        : document.querySelector(`[data-wine-category="${filterValue}"]`);
                    ...
                    if (firstVisibleSection) {
                        ...
                        const targetPosition = firstVisibleSection.getBoundingClientRect().top +
                            window.pageYOffset -
                            navbarHeight -
                            filterHeight -
                            30;
...
faqQuestions.forEach(question => {
    question.addEventListener('click', () => {
        ...
        // Toggle current FAQ only if it wasn't already expanded
        if (!isExpanded) {
